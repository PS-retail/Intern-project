      ...
      <Link to={`/${props.story.id}`}>
        ...
        <CardContent>
          <div className={classes.cardContent}>
            ...
            <br />
            <Typography variant="h7">{props.story.description}</Typography>
          </div>
        </CardContent>
      </Link>
      <CardActions disableSpacing className={classes.cardActions}>
        <IconButton aria-label="Add to Favorites" style={iconFavorite ? { color: 'red' }:{ color: 'white' }}  onClick={onClickHandler} >
          <FavoriteIcon />
        </IconButton>
      </CardActions>
    </Card>
